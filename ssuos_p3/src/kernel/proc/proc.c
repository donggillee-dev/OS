...
void init_proc()
{
	...
	list_init(&p_list); //all
	list_init(&r_list);	//run
	list_init(&s_list);	//sleep
	list_init(&d_list);	//deleted
	list_init(&f_list); //foreground
...
pid_t getValidPid(int *idx) {
	...
	for(i = 0; i < PROC_NUM_MAX; i++)
	{
		...
		if(procs[tmp % PROC_NUM_MAX].state == PROC_UNUSED) { // find out valid state;
			pid = lately_pid + 1;
			*idx = tmp % PROC_NUM_MAX; //change idx value call by address
			break;
		}
	}
...
#define TOKNUM 10
	...
	while(1)
	{
		...
		if(cur_process->pid == cur_foreground_process->pid)
			printk("> ");
		else
			continue;
...
void idle(void* aux)
{
	struct proc_option login_opt;
	login_opt.priority = 0;
	login_opt.foreground = TRUE;
	proc_create(kernel1_proc, NULL, NULL);
	proc_create(kernel2_proc, NULL, NULL);
	proc_create(login_prompt,&login_opt,NULL);
