...
void init_console(void)
{
	for(int i = 0; i < MAX_CONSOLE_NUM; i++) {
		memset(console[i].buf_s, 0, sizeof(console[i].buf_s));
		console[i].buf_w = console[i].buf_s;
		console[i].buf_p = console[i].buf_s;
		console[i].a_s = TRUE; console[i].Glob_x = 0;
		console[i].Glob_y = 2;
	}
...
{
	int i;
	for(i = cur_console->Glob_y; i > 0; i --) {
		scroll();
		set_cursor();
	}
}
...
void scroll_screen(int offset)
{
	...
	if(cur_console->sum_y < NSCROLL && offset < 0 && tmp_buf_p <= cur_console->buf_s && buf_p > buf_s) return;
